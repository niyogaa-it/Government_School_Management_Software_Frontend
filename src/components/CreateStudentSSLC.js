...
const CreateStudentsslc = () => {
    ...
    useEffect(() => {
        ...
        const fetchStudentForEdit = async () => {
            try {
                ...
                if (data.school_id) await fetchGrades(data.school_id, data.academicYear);
    ...
    const handleSchoolChange = (sid) => {
        ...
        const year = form.getFieldValue("academicYear");
    ...
    const validateDOB = (_, value) => {
        ...
        if (!value || !gradeId) return Promise.reject("Please select grade before DOB");
        ...
        const range = map[gradeName.toUpperCase()];
        if (!range) return Promise.resolve();
        const yr = new Date(value).getFullYear();
        if (yr < range[0] || yr > range[1])
            return Promise.reject(`DOB doesn't match Grade ${gradeName}`);
        return Promise.resolve();
    };
    const validateAccountNumber = (_, v) =>
        (!v || v.length < 8) ? Promise.reject("Account number must be at least 8 digits") : Promise.resolve();
    ...
    const stepFields = [
        ["school_id", "academicYear", "dateofjoin", "emisNum", "aadharNumber"],
        ["name", "gender", "grade_id", "section_id", "dob", "medium", "nationality", "state", "motherTongue",
            "hometown", "religion", "community", "identificationmarks", "bloodGroup"],
        ["fatherName", "motherName", "fatherOccupation", "motherOccupation",
            "fatherIncome", "motherIncome", "address", "mobileNumber"],
        ["academicHistory", "passorfail", "tceslc"],
        ["bankName", "branchName", "accountNumber", "ifsccode"],
    ];

    /* ── Build payload — always merges ALL stored form values (including
       unmounted step fields like school_id, academicYear from Step 0)
       using getFieldsValue(true), then overrides with the just-validated
       values so the current step's data is always fresh. ── */
    const buildPayload = (validatedValues) => {
        const allStored = form.getFieldsValue(true); // ALL steps, mounted or not
        ...
        return {
            ...
            // For superadmin use form value; for school users always use schoolId from localStorage
            school_id: role === "superadmin"
                ? (merged.school_id || schoolId)
                : schoolId,
            academicHistory: JSON.stringify(academicHistory),
            // Coerce string-sensitive fields
    ...
    const handleDraft = async () => {
        ...
        try {
            ...
            const validatedValues = await form.validateFields(currentStepFields);
            ...
            message.success(isEdit ? "Application updated successfully!" : "Draft saved successfully!");
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            // Validate only the current (last) step's fields — prior steps were
            // validated on each Next click. Then merge with all stored values.
            const validatedValues = await form.validateFields(stepFields[currentStep]);
            const payload = buildPayload(validatedValues);
            ...
            if (isEdit) navigate("/studentsslc");
            else form.resetFields();
        } catch (err) {
            message.error(err.response?.data?.error || "Failed to submit student");
        }
        setLoading(false);
    };
    ...
    const stepContent = [
        ...
        <>
            <SectionHead title="School & Academic Details" />
            {role === "superadmin" ? (
            ...
            <FI name="academicYear" label="Academic Year"
            ...
            <FI name="dateofjoin" label="Date of Join"
            ...
            <FI name="emisNum" label="EMIS Number" rules={[
                ...
                <Input placeholder="Enter EMIS number" maxLength={15} style={{ width: "100%" }}
        ...
        <>
            ...
            <FI name="grade_id" label="Grade"
                ...
                <Select placeholder="Select grade" style={{ width: "100%" }}
                    onChange={(gradeId) => {
                        ...
                        setSections([]);
                        if (sid && gradeId) fetchSectionsBySchoolAndGrade(sid, gradeId);
                    }}>
                    {grades
                        .filter(g => ["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X"]
                            .includes(g.grade?.toUpperCase()))
                        .map(g => <Option key={g.id} value={g.id}>{g.grade}</Option>)}
                </Select>
            </FI>
            <FI name="section_id" label="Section"
                rules={[{ required: true, message: "Please select section!" }]}>
                <Select placeholder="Select section" disabled={!sections.length} style={{ width: "100%" }}>
            ...
            <FI name="medium" label="Medium" rules={[{ required: true, message: "Please select medium!" }]}>
            ...
            <FI name="nationality" label="Nationality"
            ...
            <FI name="religion" label="Religion" rules={[{ required: true, message: "Required!" }]}>
                <Select placeholder="Select religion" style={{ width: "100%" }}>
                    {["Hindu", "Muslim", "Christian", "Jainism", "Others"].map(r =>
            ...
            <FI name="community" label="Community" rules={[{ required: true, message: "Required!" }]}>
                <Select placeholder="Select community" style={{ width: "100%" }}>
                    {["BC", "MBC", "SC", "ST", "OC", "OBC", "Others"].map(c =>
            ...
            <FI name="identificationmarks" label="Identification Marks"
                rules={[{ required: true, message: "Required!" }]}>
                <Input placeholder="Enter identification marks" style={{ width: "100%" }} />
            </FI>
            <FI name="bloodGroup" label="Blood Group"
                rules={[{ required: true, message: "Required!" }]}>
            ...
            <FI name="physicalDetails" label="If physically challenged, specify (otherwise enter Null)">
                <Input.TextArea autoSize={{ minRows: 3, maxRows: 5 }} maxLength={500}
                    placeholder="Enter details or 'Null'" style={{ width: "100%" }} />
            </FI>
        </>,
        ...
        <>
            ...
            <FI name="guardianNumber" label="Guardian Phone Number"
        ...
        <>
            ...
            <FI name="studentType" hidden><Input type="hidden" /></FI>
        </>,

        /* ── Step 4: Bank Details ── */
        <>
            <SectionHead title="Bank Account Information" />
            <FI name="bankName" label="Bank Name"
                rules={[{ required: true, message: "Please enter bank name!" }]}>
                <Input placeholder="Enter bank name" style={{ width: "100%" }} />
            </FI>
            <FI name="branchName" label="Branch Name"
            ...
            <div style={{ fontFamily: FF, maxWidth: 860, margin: "0 auto", paddingBottom: 40 }}>
                ...
                <div style={{ marginBottom: 22 }}>
                    <h1 style={{ fontSize: 22, fontWeight: 700, color: COLOR.text, margin: 0, letterSpacing: "-0.3px" }}>
                        {isEdit ? "Edit SSLC Student Application" : "Create SSLC Student Application"}
                    </h1>
