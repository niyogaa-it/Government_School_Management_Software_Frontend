...
// Match by school shortcode. Update this if the shortcode differs in your DB.
...
const ApplicationSSLCList = () => {
  ...
  useEffect(() => { setCurrentPage(1); }, [applicationsslcs, filterGrade, filterStatus]);

  // ── RLMHSS school detection ───────────────────────────────────────────────
  // Fee icon shows ONLY for Rani Lady Meyyammai HR Secondary School.
  // Checks shortcode first (most reliable), then school name as fallback,
  // then selectedSchoolName from filter context for superadmin view.
  const isRLMHSSContext = (app) => {
    ...
    // For non-superadmin (school login): check their own school name
    if (!isSuperAdmin) {
  ...
  const handleFeeConfirm = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}/applicationsslc/collectFee/${feeApplication.id}`,
      ...
      // Store selected date in local state so receipt icon uses correct date immediately
      setApplicationsslcs(prev => prev.map(a =>
  ...
  const handlePrintReceipt = async (app, overrideDate = null) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/applicationsslc/getApplicationsslcById/${app.id}`);
      ...
      // Single receipt block — reused twice
      const receiptBlock = () => `
  <div class="receipt">
    <div class="header">
      <div class="logo-wrap">
    ...
    <div class="receipt-title">Application Fee Receipt</div>
    <div class="meta-row">
    ...
    <div class="info-grid">
      ...
      <div class="info-cell"><div class="info-label">Medium</div><div class="info-value">${val(application.medium)}</div></div>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Page wrapper: two receipts stacked ── */
...
<div class="page">
  ...
  const handleSaveRemarks = async (appId) => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/applicationsslc/updateApplicationsslc/${appId}`, { remarks: remarksValue });
      setApplicationsslcs(prev => prev.map(a => a.id === appId ? { ...a, remarks: remarksValue } : a));
  ...
  const calculateProgress = (application) => {
    const fieldsByStep = [
      ["academicYear", "school_id", "aadharNumber"],
      ["name", "gender", "grade_id", "dob", "age", "mobileNumber", "nationality", "state", "motherTongue", "hometown", "religion", "community"],
  ...
  const openAdmitModal = async (application) => {
    ...
    // Fetch sections for this school + grade + academic year
    try {
      ...
      // First try school+grade filter, then filter by academic year client-side
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/section/getSectionsBySchoolAndGrade/${schoolId}/${gradeId}`
      );
      // Filter by academic year and active status
  ...
  const handleAdmitConfirm = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_API_URL}/applicationsslc/admit/${admitApplication.id}`,
  ...
  const handlePrint = (application) => {
    ...
    const val = (v) => (v !== null && v !== undefined && v !== "" && v !== "—") ? v : "";
...
<body>
  ...
  <div class="fields-grid">
    ...
    ${field("Is the student from scheduled tribe community?", app.tribecommunity)}
    ${field("Is the caste entitled to get ex-gratia salary?", app.exgratiasalary)}
    ${field("Is the student a convert from Hinduism to Christianity?", app.religionchanging)}
    ${field("Is the student for chicken pox? Is scar Available?", app.vaccinated)}
  ...
  <div class="fields-grid">
    ...
    ${field("Parent Email ID", app.parentEmail)}
  ...
  ${(() => {
        ...
        if (typeof history === "string") {
          ...
          // Handle double-encoded JSON (string still after first parse)
          if (typeof history === "string") { try { history = JSON.parse(history); } catch { history = []; } }
        }
        if (!Array.isArray(history) || history.length === 0) return `<div style="padding:7px 11px;font-size:12px;color:#64748b;">No academic history recorded.</div>`;
  ...
  <div class="fields-grid fields-grid-2">
   ${field("Is parent consent hardcopy attached?", app.parentconsentform)}
    ${field("Has the student passed in the last class?", app.passorfail)}
  ...
  <div style="border:1px solid #e2e8f0;border-radius:7px;overflow:hidden;margin-bottom:8px;">
    <div style="background:#fff;padding:9px 11px;min-height:48px;font-size:12px;color:#1e293b;font-weight:500;word-break:break-word;">${val(app.remarks)}</div>
  </div>
  <div class="footer">
  ...
  }, [applicationsslcs, filterGrade, filterStatus, searchQuery]);
  ...
  }, [applicationsslcs, feeHistoryFrom, feeHistoryTo]);
  ...
  const handleDownloadExcel = () => {
    ...
    const rows = filteredApps.map((a, i) => ({
      ...
      "Is the student from scheduled tribe community?": a.tribecommunity || "",
      "Is the caste entitled to get ex-gratia salary?": a.exgratiasalary || "",
      "Is the student a convert from Hinduism to Christianity?": a.religionchanging || "",
      "Is the student for chicken pox? Is scar Available?": a.vaccinated || "",
      ...
      "Parent Consent Form": a.parentconsentform || "",
      "Pass or Fail": a.passorfail || "",
  ...
  const handleDownloadFeeHistoryExcel = () => {
    ...
    rows.push({
      "S.No": "",
      "Student Name": `Total (${feeHistoryApps.length} record${feeHistoryApps.length !== 1 ? "s" : ""})`,
      "Application Number": "", "Grade": "", "Status": "", "Academic Year": "",
      "Receipt Number": "", "Payment Mode": "", "Transaction ID": "",
    ...
    ws["!cols"] = colWidths;
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Fee Collection Report");
    const dateTag = feeHistoryFrom && feeHistoryTo
      ? `_${feeHistoryFrom}_to_${feeHistoryTo}`
      : feeHistoryFrom ? `_from_${feeHistoryFrom}`
        : feeHistoryTo ? `_to_${feeHistoryTo}` : "";
    XLSX.writeFile(wb, `Fee_Collection_Report${dateTag}_${dayjs().format("YYYY-MM-DD")}.xlsx`);
    message.success(`Exported ${feeHistoryApps.length} fee records to Excel`);
  };
      ...
      <div className="app-page" style={{ fontFamily: FF }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
            ...
            <input
              ...
              style={{ paddingLeft: 32, paddingRight: searchQuery ? 30 : 12, paddingTop: 8, paddingBottom: 8, borderRadius: 8, border: `1.5px solid ${COLOR.border}`, fontSize: "13.5px", color: COLOR.text, fontFamily: FF, outline: "none", width: 240, transition: "border 0.15s" }}
          ...
          <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: `1px solid ${COLOR.border}`, marginBottom: 20, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: `1px solid ${COLOR.border}`, background: "#fffbeb" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: "15px", fontWeight: 700, color: COLOR.text }}>Fee Collection History</span>
              </div>
              {/* Total amount badge */}
                    ...
                    <tr key={app.id} onMouseEnter={() => setHoveredRow(app.id)} onMouseLeave={() => setHoveredRow(null)}
                      ...
                      <td style={{ padding: "8px 16px", textAlign: "center" }}>
                        <div style={{ display: "flex", justifyContent: "center", gap: 2 }}>
                          ...
                          {/* ── Fee Collection Button (RLMHSS only) ── */}
                          <div style={{ width: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {isRLMHSSContext(app) && (
                              app.feeCollected ? (
                                /* Receipt download icon — fee already collected */
                                <Tooltip title={`Download Receipt: ${app.receiptNumber || ""}`}>
                                  <button
                                ...
                                /* Collect fee icon — not yet collected */
                                <Tooltip title="Collect Application Fee (₹100)">
                                  <button
                          ...
                          <IconBtn icon={<PrinterOutlined />} title="Print Application" color="#7c3aed" bg="rgba(124,58,237,0.09)"
                            onClick={async () => {
                              ...
                              } catch { message.error("Failed to fetch application"); }
                            }}
                          />
                          ...
                          <div style={{ width: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {isSuperAdmin && app.studentStatus !== "Admitted" && (
                              <IconBtn icon={<DeleteOutlined />} title="Remove Application" color={COLOR.danger} bg={COLOR.dangerBg} onClick={() => handleDelete(app.id, app.name)} />
                            )}
                          </div>
                        </div>
                      </td>
        ...
        {isModalVisible && selectedApplication && (
          <Modal
            ...
            <Descriptions bordered column={2} size="small"
              ...
              <Descriptions.Item label="Parent Email ID">{selectedApplication.parentEmail}</Descriptions.Item>
              ...
              <Descriptions.Item label="Has student passed last class?">{selectedApplication.passorfail}</Descriptions.Item>
              ...
              <Descriptions.Item label="Remarks" span={3}>{selectedApplication.remarks || "—"}</Descriptions.Item>
            </Descriptions>

            {/* ── Academic History – rendered OUTSIDE <Descriptions> to prevent layout break ── */}
            {(() => {
                  ...
                  {history.length === 0 ? (
                    ...
                    <div style={{ border: `1px solid ${COLOR.border}`, borderTop: "none", borderRadius: "0 0 5px 5px", overflow: "hidden" }}>
                      ...
                      <div style={{
                        ...
                        background: "#f1f5f9", borderBottom: `1px solid ${COLOR.border}`
                      }}>
        ...
        {feeModalVisible && feeApplication && (
          <Modal
            title={
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontFamily: FF, fontWeight: 700, fontSize: 16, color: COLOR.text }}>Collect Application Fee</span>
              </div>
            }
          ...
          >
            {/* Student info card */}
            <div style={{ background: "#f8fafc", border: `1px solid ${COLOR.border}`, borderRadius: 10, padding: "14px 18px", marginBottom: 18 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 20px" }}>
                ...
                <div>
                  <div style={{ fontSize: "11px", color: COLOR.textSoft, fontWeight: 600, marginBottom: 2 }}>Application No</div>
            ...
            {/* Fee amount display */}
            <div style={{ background: "#eff6ff", border: "1.5px solid #3b82f6", borderRadius: 8, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
              <div style={{ fontSize: "13.5px", fontWeight: 600, color: "#1e40af" }}>Application Fee</div>
              <div style={{ fontSize: "22px", fontWeight: 800, color: "#1e40af" }}>₹ {FEE_AMOUNT}.00</div>
            </div>

            {/* Payment mode selection */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: "13.5px", fontWeight: 600, color: COLOR.text, marginBottom: 10 }}>
                Payment Mode <span style={{ color: COLOR.danger }}>*</span>
              </div>
              <div style={{ display: "flex", gap: 8, background: "#f1f5f9", borderRadius: 10, padding: 4 }}>
                {[
                  ...
                  <button
                    ...
                    style={{
                      all: "unset", flex: 1, padding: "8px 0", borderRadius: 7, textAlign: "center",
                      fontSize: "13.5px", fontWeight: 600, cursor: "pointer", transition: "all 0.15s",
                      background: feePaymentMode === mode ? "#fff" : "transparent",
                      color: feePaymentMode === mode ? "#1e40af" : COLOR.textSoft,
                      boxShadow: feePaymentMode === mode ? "0 1px 4px rgba(0,0,0,0.10)" : "none"
                    }}
                  >
                    {label}
                  </button>
            ...
            {feePaymentMode === "online" && (
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: "13.5px", fontWeight: 600, color: COLOR.text, marginBottom: 6 }}>
                  Transaction ID <span style={{ color: COLOR.danger }}>*</span>
                </div>
                <input
                  type="text"
                  ...
                  style={{
                    width: "100%", padding: "9px 12px", borderRadius: 8, fontSize: "13.5px",
                    border: `1.5px solid ${COLOR.border}`, outline: "none", fontFamily: FF,
                    color: COLOR.text, background: "#fff", boxSizing: "border-box", transition: "border 0.15s"
                  }}
                  onFocus={e => { e.target.style.borderColor = "#3b82f6"; }}
                  onBlur={e => { e.target.style.borderColor = COLOR.border; }}
                />
        ...
        {admitModalVisible && admitApplication && (
          <Modal
            ...
            onCancel={() => { setAdmitModalVisible(false); setAdmitApplication(null); setAdmitSectionId(null); }}
            footer={null}
            width={560}
            centered
          >
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              {sectionsLoading ? (
                ...
                <div style={{ padding: "10px 14px", background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 8, color: "#c2410c", fontSize: "13px" }}>
                  No sections found for <strong>{admitApplication.Grade?.grade}</strong> in academic year <strong>{admitApplication.academicYear}</strong>. Please create a section first.
                </div>
              ) : (
                <Select
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
              <button
                onClick={() => { setAdmitModalVisible(false); setAdmitApplication(null); setAdmitSectionId(null); }}
