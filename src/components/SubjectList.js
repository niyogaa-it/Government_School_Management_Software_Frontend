...
const SubjectList = () => {
  ...
  useEffect(() => { fetchSubjects(); }, [selectedSchool, selectedYear, location.pathname]);
  ...
  const fetchSubjects = async () => {
    try {
      ...
      if (schoolId && selectedYear) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/subject/getSubjectsBySchoolAndYear/${schoolId}/${selectedYear}`);
      } else if (schoolId) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/subject/getSubjectsBySchool/${schoolId}`);
      } else {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/subject/getAllSubjects`);
      }
      let data = (res.data.subjects || []).filter(s => s.status !== 0).sort((a,b) => new Date(b.createdAt)-new Date(a.createdAt));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/subject/updateStatus/${id}`, { status: 0 });
      message.success("Subject deleted successfully"); fetchSubjects();
    } catch { message.error("Failed to delete subject"); }
  };

  const showFilter = selectedSchool !== "all" || selectedYear;
  const cols = ["S.No","School","Academic Year","Grade","Subject Name","Short Code","Action"];

  return (
    <Layout>
      <div className="app-page" style={{ fontFamily:FF }}>
        <div style={{ marginBottom:22 }}>
          <h1 style={{ fontSize:22, fontWeight:700, color:COLOR.text, margin:0, letterSpacing:"-0.3px" }}>Subject List</h1>
        ...
        <div style={{ background:"#fff", borderRadius:12, boxShadow:"0 2px 12px rgba(0,0,0,0.07)", overflow:"hidden", border:`1px solid ${COLOR.border}` }}>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontFamily:FF, fontSize:FS }}>
              ...
              <tbody>
                {subjects.length > 0 ? subjects.map((s, index) => (
                  <tr key={s.id} onMouseEnter={() => setHoveredRow(s.id)} onMouseLeave={() => setHoveredRow(null)}
                    ...
                    <td style={{ padding:"11px 16px", color:COLOR.text, fontWeight:500 }}>{s.subjectName}</td>
                    <td style={{ padding:"11px 16px", color:COLOR.textMid }}>{s.shortCode}</td>
                    <td style={{ padding:"8px 16px", textAlign:"center" }}>
                      <div style={{ display:"flex", justifyContent:"center", gap:4 }}>
                        <IconBtn icon={<EyeOutlined />}  title="View Subject"   color={COLOR.blue}      bg={COLOR.viewBg}   onClick={() => { setSelectedSubject(s); setIsModalVisible(true); }} />
                        <IconBtn icon={<EditOutlined />} title="Edit Subject"   color={COLOR.editColor} bg={COLOR.editBg}   onClick={() => navigate(`/edit-subject/${s.id}`)} />
                        {isSuperAdmin && <IconBtn icon={<DeleteOutlined />} title="Delete Subject" color={COLOR.danger} bg={COLOR.dangerBg} onClick={() => handleDelete(s.id)} />}
                      </div>
                    </td>
                  </tr>
                )) : (
                  <tr><td colSpan={7} style={{ textAlign:"center", padding:"40px 16px", color:COLOR.textSoft, fontSize:FS }}>No subjects found{selectedYear ? ` for ${selectedYear}` : ""}.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {selectedSubject && (
          <Modal title={<span style={{ fontFamily:FF, fontWeight:700, fontSize:16, color:COLOR.text }}>Subject Details</span>}
            ...
            <Descriptions bordered column={1} size="small"
              ...
              <Descriptions.Item label="School">{selectedSubject.School?.name||selectedSchoolName||"N/A"}</Descriptions.Item>
              <Descriptions.Item label="Academic Year">{selectedSubject.academic_year||"N/A"}</Descriptions.Item>
              <Descriptions.Item label="Grade">{selectedSubject.Grade?.grade||"N/A"}</Descriptions.Item>
              <Descriptions.Item label="Subject Name">{selectedSubject.subjectName}</Descriptions.Item>
              <Descriptions.Item label="Short Code">{selectedSubject.shortCode}</Descriptions.Item>
            </Descriptions>
          </Modal>
        )}
      </div>
