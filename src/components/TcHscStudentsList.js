...
const TcHscStudentsList = () => {
  ...
  // ── Fetch list ───────────────────────────────────────────────────────────
  // Logic:
  //   superadmin + specific school + year  → getTcsBySchoolAndYear/:id/:year
  //   superadmin + specific school         → getTcsBySchool/:id
  //   superadmin + all schools + year      → getAllTcs  then client-filter by year
  //   superadmin + all schools             → getAllTcs
  //   school admin                         → getTcsBySchool/:schoolId  (always scoped)
  const fetchTcList = async () => {
    try {
      ...
      if (effectiveSchoolId && selectedYear) {
        // Specific school + specific year
        res = await axios.get(
          `${process.env.REACT_APP_API_URL}/tchsc/getTcsBySchoolAndYear/${effectiveSchoolId}/${selectedYear}`
        );
      } else if (effectiveSchoolId) {
        // Specific school, all years
        res = await axios.get(
          `${process.env.REACT_APP_API_URL}/tchsc/getTcsBySchool/${effectiveSchoolId}`
        );
      } else {
        // All schools
        res = await axios.get(
          `${process.env.REACT_APP_API_URL}/tchsc/getAllTcs`
        );
      }

      let data = res.data.tcs || [];

      // Client-side year filter for "all schools + year" path
      if (isSuperAdmin && selectedSchool === "all" && selectedYear) {
        data = data.filter(t => t.academicYear === selectedYear);
      }
      ...
      console.error(err);
      message.error(err.response?.data?.error || "Failed to fetch HSC TC students");
    }
  };
  ...
  const handleView = async (id) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/tchsc/getTcById/${id}`);
  ...
  const handleCancelTc = async (id, name) => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/tchsc/cancelTc/${id}`);
  ...
  const handlePrint = (tc) => {
    ...
    pw.document.title = `TC-HSC - ${tc.tcNumber}`;
      ...
      <div className="app-page" style={{ fontFamily: FF }}>
        ...
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", overflow: "hidden", border: `1px solid ${COLOR.border}` }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: FF, fontSize: FS, minWidth: 900 }}>
              <thead>
                ...
                {filtered.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={columns.length} style={{ textAlign: "center", padding: "48px 16px", color: COLOR.textSoft, fontSize: FS }}>
                      ...
                      No HSC TC records found{selectedYear ? ` for ${selectedYear}` : ""}.
                    </td>
                  </tr>
                )}
              </tbody>
...
export default TcHscStudentsList;