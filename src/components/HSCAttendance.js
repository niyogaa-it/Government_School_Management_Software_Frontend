...
const HSCAttendance = () => {
  ...
  // ── HSC grades: only XI and XII ───────────────────────────────────────────
  const [grades, setGrades] = useState([]);
  const [gradesLoading, setGradesLoading] = useState(false);
  const [noGrades, setNoGrades] = useState(false);   // true when API returns []
  ...
  // ── Load HSC grades (XI & XII only) when school+year changes ──────────────
  useEffect(() => {
    if (!selSchoolId || !academicYear) {
      setGrades([]); setSelGradeId(""); setSections([]); setSelSectionId("");
      setNoGrades(false); return;
    }
    setGradesLoading(true);
    setSelGradeId(""); setSections([]); setSelSectionId(""); setNoGrades(false);

    axios.get(`${process.env.REACT_APP_API_URL}/attendancehsc/getHSCGrades`, {
      params: { school_id: selSchoolId, academicYear },
    })
      .then(r => {
        const g = r.data.grades || [];
        setGrades(g);
        setNoGrades(g.length === 0);
      })
      .catch(() => { setGrades([]); setNoGrades(true); })
      .finally(() => setGradesLoading(false));
  }, [selSchoolId, academicYear]);

  // ── Load sections when grade changes ──────────────────────────────────────
  useEffect(() => {
    if (!selSchoolId || !selGradeId || !academicYear) {
      setSections([]); setSelSectionId(""); return;
    }
  ...
  const handleGo = useCallback(async () => {
    ...
    try {
      const [studRes, eventHolidays] = await Promise.all([
        axios.get(`${process.env.REACT_APP_API_URL}/attendancehsc/getStudentsForAttendance`,
      ...
      await Promise.all(cols.map(async (col) => {
        ...
        try {
          const r = await axios.get(`${process.env.REACT_APP_API_URL}/attendancehsc/getAttendance`,
  ...
  // ── Session time window restriction (non-superadmin) ──────────────────────
  ...
  const checkSessionTime = (sess) => {
    ...
    if (!isWithinSessionTime(sess)) {
      ...
      message.warning({ content: `Attendance time is over! ${sess} session can only be marked between ${win.label || ""}. Please contact the Super Admin.`, duration: 5 });
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    if (isAttHoliday(selDate)) { message.warning("This date is marked as a Holiday. Attendance cannot be submitted."); return; }
    ...
    setSubmitting(true);
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/attendancehsc/submitAttendance`, {
  ...
  const fetchMonthly = useCallback(async () => {
    ...
    try {
      const studRes = await axios.get(`${process.env.REACT_APP_API_URL}/attendancehsc/getStudentsForAttendance`,
        { params: { school_id: selSchoolId, academicYear, grade_id: selGradeId, section_id: selSectionId } });
      const studs = studRes.data.students || [];
        ...
        <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: `1px solid ${C.border}`, padding: "18px 24px", marginBottom: 20 }}>
          <div className="att-filter-grid">
            ...
            <div>
              ...
              {gradesLoading ? (
                <div style={{ ...readonlyStyle, display: "flex", alignItems: "center", gap: 6, color: C.textSoft }}>
                  <Spin size="small" /> Loading…
                </div>
              ) : noGrades ? (
                <div style={{ ...readonlyStyle, color: "#dc2626", display: "flex", alignItems: "center", gap: 6 }}>
                  ⚠ No grades available
                </div>
              ) : (
                <Select style={{ width: "100%" }} placeholder="Select Class (XI / XII)"
            ...
            <div>
              ...
              {activeTab === "attendance" ? (
                <Select style={{ width: "100%" }} value={session} onChange={v => { setSession(v); setLoaded(false); }}>
            ...
            {activeTab === "attendance" && (
              <div style={{ display: "flex", alignItems: "flex-end" }}>
                <button onClick={handleGo}
                  disabled={!selSchoolId || !selGradeId || !selSectionId || loading || noGrades}
                  style={{ width: "100%", height: 32, borderRadius: 7, fontFamily: FF, background: (!selSchoolId || !selGradeId || !selSectionId || loading || noGrades) ? "#94a3b8" : C.blue, color: "#fff", fontWeight: 700, fontSize: 13, border: "none", cursor: (!selSchoolId || !selGradeId || !selSectionId || loading || noGrades) ? "not-allowed" : "pointer" }}>
                  {loading ? "..." : "Go"}
                </button>
              </div>
            )}
          ...
          <>
            ...
            {!loaded && !loading && (
              <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "60px 20px", textAlign: "center", color: C.textSoft, fontSize: 14 }}>
                {noGrades
                  ? <span>No HSC grades (XI / XII) are available for the selected school and academic year.</span>
                  : <span>Select the filters above and click <strong>Go</strong> to load the attendance sheet.</span>
                }
              </div>
            )}
          </>
          ...
          <Spin spinning={monthlyLoading}>
            {monthlyData ? (
              ...
              <div style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, padding: "60px 20px", textAlign: "center", color: C.textSoft, fontSize: 14 }}>
                ...
                {noGrades
                  ? <span>No HSC grades (XI / XII) are available for the selected school and academic year.</span>
                  : <span>Select School, Class, Section &amp; Month above then click <strong>Go</strong>.</span>
                }
              </div>
            )}
          </Spin>
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", border: `1px solid ${C.border}`, overflow: "hidden" }}>
      <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <span style={{ fontWeight: 700, fontSize: 15, color: C.text }}>{MN[month]} {year} — HSC Attendance</span>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {isAdmin && (
            <span style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic" }}>
              Click 📅 to manually toggle holiday · Auto-holidays from events are pre-marked
            </span>
          )}
        </div>
      </div>
