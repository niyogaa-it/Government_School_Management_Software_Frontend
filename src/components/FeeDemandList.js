...
/* ── Semantic icon-button color pairs ── */
const COLOR = {
  payColor: "#16a34a",
...
const printReceipt = (record, school) => {
  ...
  const currentPayment = parseFloat(record.current_payment || record.paid_amount || 0);
  ...
  const logoHTML = schoolLogo
    ? `<img src="${schoolLogo}" alt="School Logo" style="height:64px;width:64px;object-fit:contain;flex-shrink:0;" />`
    : "";

  // A4 Portrait — 2 identical receipts, single page
  const receiptBlock = `
    <div class="rw">
      <div class="hd">
        ${logoHTML ? `<img src="${schoolLogo}" alt="logo" style="height:44px;width:44px;object-fit:contain;flex-shrink:0;"/>` : ""}
        <div class="hd-info">
      ...
      <div class="bd">
        ...
        <table>
          <thead><tr><th style="width:26px">S.No</th><th>Fee Type</th><th style="text-align:right">Amount (₹)</th></tr></thead>
          <tbody>
            ${feeItemsRows}
            <tr><td colspan="3" style="padding:1px 0;border:none;background:transparent"></td></tr>
            ${totalFeeRow}
            ${alreadyPaidRow}
            ${balanceToPayRow}
            ${currentPaymentRow}
  ...
  const receiptHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
    <title>Fee Receipt - ${record.receipt_no || "Receipt"}</title>
    <style>
      *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;}
      ...
      /* Each receipt = exactly half of usable page height */
      /* page height 297mm - top+bottom padding 8mm - cut line 10mm = 279mm / 2 = 139.5mm each */
       .rw{
        width:100%;
        height:calc((100% - 10mm) / 2);
        background:#fff;
      ...
      /* Cut line — exactly 10mm height with visible scissor dashes */
      .cut-line{
        height:10mm;
        flex-shrink:0;
        display:flex;
      ...
      /* Header */
      .hd{
        background:linear-gradient(135deg,#1d2a4d 0%,#2d4073 100%)!important;
      ...
      .sn{font-size:12.5px;font-weight:800;letter-spacing:0.8px;text-transform:uppercase;margin-bottom:1px;}
      .sa{font-size:7.5px;opacity:0.85;margin-bottom:3px;}
      .rb{display:inline-block;background:rgba(255,255,255,0.2)!important;border:1px solid rgba(255,255,255,0.5);color:white!important;font-size:7.5px;font-weight:700;letter-spacing:2px;padding:1px 9px;border-radius:20px;text-transform:uppercase;}

      /* Body — flex column, NO gap so footer hugs table */
      .bd{
      ...
      /* Receipt meta row */
      .rm{display:flex;justify-content:space-between;align-items:center;padding-bottom:4px;border-bottom:1px dashed #e0e0e0;flex-shrink:0;}
      .rn,.rd{font-size:9px;color:#555;}
      .rn span,.rd span{font-weight:700;color:#1d2a4d;font-size:9.5px;}

      /* Student info grid */
      .ig{display:grid;grid-template-columns:1fr 1fr;gap:2px 12px;background:#f8f9ff!important;border-radius:5px;padding:5px 9px;flex-shrink:0;}
      .ir{display:flex;flex-direction:column;gap:0;}
      .il{font-size:6.5px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:0.5px;}
      .iv{font-size:9px;font-weight:600;color:#222;}

      /* Fee table */
      table{width:100%;border-collapse:collapse;font-size:9px;flex-shrink:0;}
      thead tr{background:#1d2a4d!important;color:white!important;}
      th{padding:3px 7px;text-align:left;font-weight:600;font-size:8px;color:white!important;}
      tbody tr:nth-child(even){background:#f5f7ff!important;}
      td{padding:3px 7px;border-bottom:1px solid #eee;color:#333;}

      /* Summary rows compact */
      tr.sum td{padding:4px 7px!important;}

      /* Footer — no margin-top:auto, sits right after table */
      .ft{
        display:flex;
      ...
      .ft-note{font-size:6.5px;color:#aaa;line-height:1.6;}
      .ft-sign{width:95px;border-top:1px solid #555;padding-top:3px;font-size:8px;color:#666;font-weight:600;text-align:center;}

      @media print{
        html,body{background:white;overflow:hidden;}
        .page{background:white;padding:4mm 5mm;}
        .rw{box-shadow:none;}
...
const PaymentModal = ({ demand, school, onClose, onSaved, notifApi }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (typeof demandFeeItems === "string") {
        try { demandFeeItems = JSON.parse(demandFeeItems); } catch { demandFeeItems = []; }
      }
      ...
      if (typeof collectionFeeItems === "string") {
        try { collectionFeeItems = JSON.parse(collectionFeeItems); } catch { collectionFeeItems = []; }
      }
      ...
      const enrichedCollection = {
        ...
        admission_number: collection.admission_number || demand.admission_number,
  ...
  if (typeof feeItems === "string") {
    try { feeItems = JSON.parse(feeItems); } catch { feeItems = []; }
  }
...
const ReceiptHistoryModal = ({ demand, collections, school, onClose }) => {
  ...
  const handlePrint = (col) => {
    ...
    if (typeof colFeeItems === "string") {
      try { colFeeItems = JSON.parse(colFeeItems); } catch { colFeeItems = []; }
    }
...
export default function FeeDemandList() {
  ...
  const filteredGradeOptions = gradeOptions.filter((g) => {
    ...
    if (filterCourse === "HSC") {
      return (
        gradeName === "XI" || gradeName === "XII" ||
        gradeName === "11" || gradeName === "12"
      );
    }
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE}/section/getSectionsBySchoolAndGrade/${schoolId}/${filterGrade}`)
  ...
  const fetchDemands = useCallback(async () => {
    ...
    try {
      ...
      if (filterGrade) {
        const gl = gradeOptions.find((g) => g.id === parseInt(filterGrade))?.grade;
        if (gl) params.set("grade", gl);
      }
      if (filterSection) {
        const sl = sectionOptions.find((s) => s.id === parseInt(filterSection))?.sectionName;
    ...
    schoolId, filterYear, filterCourse, filterGrade,
    filterSection, filterStatus, gradeOptions, sectionOptions,
  ]);
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [filterSearch, filterYear, filterCourse, filterGrade, filterSection, filterStatus]);
  ...
  const handleExportExcel = () => {
    const headers = [
      "S.No", "Student Name", "Admission No.", "Grade", "Section",
      "Medium", "Type", "Total", "Paid", "Balance", "Status",
    ];
    const rows = visible.map((d, i) => [
      i + 1, d.student_name || "", d.admission_number || "",
      d.grade || "", d.section || "", d.medium || "", d.student_type || "",
  ...
  /* ── buildReceiptHTML for bulk — A5 Portrait ── */
    ...
    try {
      ...
      for (const demand of paid) {
        ...
        for (const col of cols) {
          ...
          if (!Array.isArray(colFeeItems)) colFeeItems = [];
          const feeItems = demandFeeItems.length > 0 ? demandFeeItems : colFeeItems;
      ...
      const combinedHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
        ...
        <style>
          *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;}
          ...
          .hd{background:linear-gradient(135deg,#1d2a4d 0%,#2d4073 100%)!important;color:white!important;padding:16px 20px 14px;display:flex;align-items:center;gap:14px;position:relative;}
          ...
          .sn{font-size:16px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:3px;}
          .sa{font-size:10px;opacity:0.85;margin-bottom:8px;}
          .rb{display:inline-block;background:rgba(255,255,255,0.2)!important;border:1.5px solid rgba(255,255,255,0.5);color:white!important;font-size:10px;font-weight:600;letter-spacing:3px;padding:3px 16px;border-radius:20px;text-transform:uppercase;}
          .bd{padding:20px 22px 16px;flex:1;display:flex;flex-direction:column;gap:10px;}
          .rm{display:flex;justify-content:space-between;align-items:center;padding-bottom:10px;border-bottom:2px dashed #e0e0e0;}
          .rn{font-size:11px;color:#555;}.rn span{font-weight:700;color:#1d2a4d;font-size:13px;}
          .rd{font-size:11px;color:#555;text-align:right;}.rd span{font-weight:700;color:#333;}

          /* Student info — 2 cols */
          .ig{display:grid;grid-template-columns:1fr 1fr;gap:6px 18px;background:#f8f9ff!important;border-radius:8px;padding:12px 14px;}
          .ir{display:flex;flex-direction:column;gap:1px;}
          .il{font-size:8px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:0.8px;}
          .iv{font-size:11px;font-weight:600;color:#222;}

          table{width:100%;border-collapse:collapse;font-size:11px;}
          thead tr{background:#1d2a4d!important;color:white!important;}
          th{padding:7px 9px;text-align:left;font-weight:600;font-size:10px;color:white!important;}
          tbody tr:nth-child(even){background:#f5f7ff!important;}
          td{padding:6px 9px;border-bottom:1px solid #eee;color:#333;}
          ...
          .row-current{background:#e8eaf6!important;} .row-current td{color:#1d2a4d!important;font-weight:700;font-size:12px;}
          ...
          .footer-row{margin-top:auto;padding-top:12px;border-top:1.5px dashed #ddd;display:flex;justify-content:space-between;align-items:flex-end;}
          .footer-note{font-size:8.5px;color:#999;line-height:1.8;flex:1;}
          ...
          .sig-line{width:120px;border-top:1.5px solid #555;padding-top:5px;font-size:9px;color:#666;font-weight:600;text-align:center;}
          .stamp-paid{border:3px solid #27ae60!important;color:#27ae60!important;font-weight:800;font-size:18px;padding:5px 14px;border-radius:6px;transform:rotate(-12deg);letter-spacing:3px;display:inline-block;}
          .stamp-partial{border:3px solid #e67e22!important;color:#e67e22!important;font-weight:800;font-size:14px;padding:5px 12px;border-radius:6px;transform:rotate(-12deg);letter-spacing:2px;display:inline-block;}
    ...
    <Layout>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 18, flexWrap: "wrap" }}>
          {[
            { label: "Total Demands", value: stats.total, color: C.accent, bg: C.accentLight },
            { label: "Unpaid", value: stats.unpaid, color: C.danger, bg: "#fee2e2" },
            { label: "Partial", value: stats.partial, color: "#8b5cf6", bg: "#ede9fe" },
            { label: "Paid", value: stats.paid, color: C.success, bg: C.successLight },
            {
              label: "Outstanding",
            ...
            >
              <span style={{ fontSize: 11, color: C.muted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                {label}
              </span>
        ...
        >
          <div
            style={{
              ...
              marginBottom: 16, paddingBottom: 10, borderBottom: "1.5px solid #e8f0fe",
            }}
          >
            Filter Demands
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 14, alignItems: "end" }}>
            ...
            <div>
              ...
              <Select value={filterYear} onChange={(val) => setFilterYear(val)} style={{ width: "100%", fontFamily: FF }} size="middle">
            ...
            <div>
              ...
              <Select
                value={filterCourse || undefined} placeholder="All Courses" allowClear
                onChange={(val) => { setFilterCourse(val || ""); setFilterGrade(""); setFilterSection(""); }}
            ...
            <div>
              ...
              <Select
                value={filterGrade || undefined} placeholder="All Grades" allowClear
                onChange={(val) => { setFilterGrade(val || ""); setFilterSection(""); }}
                style={{ width: "100%", fontFamily: FF }}
              >
                {filteredGradeOptions.map((g) => <Option key={g.id} value={g.id}>{g.grade}</Option>)}
              </Select>
            </div>
            {/* Section */}
            <div>
              ...
              <Select
                value={filterSection || undefined} placeholder="All Sections" allowClear
                disabled={!filterGrade}
                onChange={(val) => setFilterSection(val || "")}
                style={{ width: "100%", fontFamily: FF }}
              >
                {sectionOptions.map((s) => <Option key={s.id} value={s.id}>{s.sectionName}</Option>)}
              </Select>
            </div>
            {/* Status */}
            <div>
              ...
              <Select
                value={filterStatus || undefined} placeholder="All Statuses" allowClear
        ...
        <div
          style={{
            ...
            boxShadow: "0 2px 12px rgba(0,0,0,0.07)",
            overflow: "hidden", border: `1px solid ${COLOR.border}`,
          }}
        >
          <div
            style={{
          ...
          >
            <span style={{ fontSize: 14, fontWeight: 700, color: C.primary }}>Fee Demands</span>
          ...
          {loading ? (
            ...
            <div style={{ padding: 60, textAlign: "center", color: C.muted }}>
              <FileTextOutlined style={{ fontSize: 36, color: C.border, display: "block", marginBottom: 10 }} />
              <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>No demands found</div>
              <div style={{ fontSize: 13, marginBottom: 16 }}>
                {demands.length === 0 ? "No demands have been raised yet." : "No demands match the current filters."}
              </div>
              <button
                onClick={() => navigate("/raisestudentdemand")}
                style={{
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="fdl-tbl">
                    ...
                    {paginated.map((d, i) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{ background: baseBg, transition: "background 0.12s", borderBottom: `1px solid ${COLOR.border}` }}
                        >
                          <td style={{ whiteSpace: "nowrap", textAlign: "center", padding: "8px 16px" }}>
                            <div style={{ display: "flex", justifyContent: "center", gap: 2 }}>
                              ...
                              {d.status !== "Unpaid" && (
                                <IconBtn
                                  icon={
                                    printingId === d.id
                                      ? <span className="fdl-spinner" style={{ width: 14, height: 14 }} />
                                      : <PrinterOutlined />
                                  }
                                  title="View Receipts"
                          ...
                          <td style={{ color: C.muted, fontWeight: 600 }}>{globalIdx + 1}</td>
                          <td style={{ fontWeight: 600 }}>{d.student_name}</td>
                          <td style={{ fontFamily: "monospace", fontSize: 12, color: C.muted }}>{d.admission_number}</td>
                          <td style={{ textAlign: "center" }}>{d.grade || "—"}</td>
                          <td style={{ textAlign: "center" }}>{d.section || "—"}</td>
                          <td>{d.medium || "—"}</td>
                          <td>
                            <span style={{ display: "inline-block", padding: "2px 9px", borderRadius: 12, fontSize: 11, fontWeight: 700, background: C.accentLight, color: C.accent }}>
                              {d.student_type || "—"}
                            </span>
                          </td>
                          <td style={{ textAlign: "right", fontWeight: 700, color: C.primary, fontVariantNumeric: "tabular-nums" }}>
                            {totalAmt.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                          </td>
                          <td style={{ textAlign: "right", fontWeight: 700, color: "#166534", fontVariantNumeric: "tabular-nums" }}>
                            {paidAmt > 0 ? paidAmt.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "—"}
                          </td>
                          <td style={{ textAlign: "right", fontWeight: 700, fontVariantNumeric: "tabular-nums", color: balance > 0 ? C.danger : C.muted }}>
                  ...
                  {visible.length > 0 && (
                    <tfoot>
                      <tr>
                        <td colSpan={8} style={{ textAlign: "right", fontWeight: 700, color: "#166534", padding: "11px 13px" }}>
                          Total ({visible.length} records)
                        </td>
                        <td style={{ textAlign: "right", fontWeight: 800, fontSize: 14, color: "#166534", fontVariantNumeric: "tabular-nums", padding: "11px 13px" }}>
                          {stats.totalDue.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                        </td>
                        <td style={{ textAlign: "right", fontWeight: 700, color: "#166534", fontVariantNumeric: "tabular-nums", padding: "11px 13px" }}>
                          {stats.totalPaid.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                        </td>
                        <td style={{ textAlign: "right", fontWeight: 700, color: C.warning, fontVariantNumeric: "tabular-nums", padding: "11px 13px" }}>
                          {(stats.totalDue - stats.totalPaid).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                        </td>
                        <td />
                      </tr>
                    </tfoot>
              ...
              {visible.length > 0 && (
                ...
                >
                  <span style={{ fontSize: 13, color: COLOR.textSoft, fontFamily: FF }}>
                    Showing{" "}
                    <strong>{visible.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}</strong>
                    –<strong>{Math.min(currentPage * PAGE_SIZE, visible.length)}</strong>{" "}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <button
                      ...
                      style={{
                        all: "unset", width: 32, height: 32, borderRadius: 6,
                        display: "flex", alignItems: "center", justifyContent: "center",
                        cursor: currentPage === 1 ? "not-allowed" : "pointer",
                        background: currentPage === 1 ? "#f0f0f0" : "#fff",
                        border: `1px solid ${COLOR.border}`,
                        color: currentPage === 1 ? "#c0c0c0" : COLOR.textMid, fontSize: 13,
                      }}
                    >
                      <LeftOutlined />
                    </button>
                        ...
                        page === "..." ? (
                          ...
                          <button
                            ...
                            style={{
                              ...
                              background: currentPage === page ? "#1a2236" : "#fff",
                              color: currentPage === page ? "#fff" : COLOR.textMid,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${COLOR.border}`,
                        color: currentPage === totalPages ? "#c0c0c0" : COLOR.textMid, fontSize: 13,
                      }}
                    >
                      <RightOutlined />
                    </button>
